...
def run(Context):
    ...
    for td in Context.date_range['trade_date']:
        ...
        g.state = next_state
        next_action = g.agent.get_action(next_state)-1
        next_state = np.reshape(next_state, [1, 3])
        g.state = np.reshape(g.state, [1, 3])
...
def plot_return(Context):
    ...
    #plt.semilogy()
    #plt.show()
    plt.savefig('./fig/'+str(g.num)+'.png',bbox_inches='tight', dpi=256)
    plt.close()
...
class DeepSARSAgent:
    def __init__(self):
        ...
        self.state_size = 3 # 环境状态等数据
        self.discount_factor = 0.99
        self.learning_rate = 0.001

        self.epsilon = 2.  # exploration
...
def handle(Context,td,value):
    ...
    if len(history) == 0:
        ...
        if len(g.state) == 0:
            g.state = [(history['close'][-3]-history['close'][-5])/history['close'][-5]\
                       ,(history['close'][-3]-2*history['close'][-7]+history['close'][-12])/history['close'][-7]]#history['close'].tolist()[-12:-2]
            g.state.append(0)
            sig = g.agent.get_action(g.state)-1
        else:
            sig = g.agent.get_action(g.state)-1
        reward = (history['close'][-1]-history['close'][-2])/history['close'][-2]
        # 输出的next state
        state = [(history['close'][-1]-history['close'][-3])/history['close'][-3]\
                       ,(history['close'][-1]-2*history['close'][-5]+history['close'][-10])/history['close'][-5]]#history['close'].tolist()[-10:]
...
if __name__ == "__main__":
    ...
    Init(C)
    g.num = 0
    for g.num in range(g.epi):

        C = Context(100000,'2020-01-10','2023-01-31','hfq',"000099")
        Return = run(C)

        g.scores.append(g.score)
        g.episodes.append(g.num)
        pylab.plot(g.episodes, g.scores, 'b')
        pylab.savefig("./save_graph/deep_sarsa_.png")
        print("episode:", g.num, "  score:", g.score, "global_step",
